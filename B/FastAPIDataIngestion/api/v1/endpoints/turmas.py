...
#Tentativa de Patch
@router.patch("/{turma_id}", response_model=TurmaSchema, status_code=status.HTTP_202_ACCEPTED)
async def patch_turma(turma_id: int, turma: TurmaSchema, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(TurmasModel).filter(TurmasModel.id == turma_id)
        result = await session.execute(query)
        turma_up = result.scalar_one_or_none()
        ...
        if turma_up:
            if turma_up.nome_turma != None:
                turma_up.nome_turma = turma.nome_turma
            if turma_up.padrinho != None:
                turma_up.padrinho = turma.padrinho
            if turma_up.qtd_alunos != None:
                turma_up.qtd_alunos = turma.qtd_alunos
            if turma_up.laboratorio != None:
                turma_up.laboratorio = turma.laboratorio
